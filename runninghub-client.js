...
export class RunningHubClient {
  ...
  /**
   * 上传图片到RunningHub
   * @param {string} imageBase64 - Base64编码的图片（可以是 data:image/...;base64,xxx 格式或纯 base64）
  ...
  async uploadImage(imageBase64) {
    ...
    if (imageBase64.startsWith('data:')) {
      ...
      if (base64Index !== -1) {
        base64Data = imageBase64.substring(base64Index + 7); // 'base64,'.length = 7
      }
    }
  ...
  async queryTaskOutput(taskId) {
    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt++) {
      try {
        ...
        } else if (data.msg === "APIKEY_TASK_STATUS_ERROR" || data.msg.includes("ERROR")) {
          // 任务出错，打印详细信息并抛出错误
          console.error('❌ 任务执行出错，详细信息:', JSON.stringify(data, null, 2));
          throw new Error(`任务执行出错: ${data.msg}。详细信息: ${JSON.stringify(data.data || {})}`);
        } else {
          // 其他状态，继续等待
